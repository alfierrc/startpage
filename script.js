...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const openArchiveBtn = document.getElementById('archive-open-btn');
  const closeArchiveBtn = document.getElementById('archive-close-btn');
  const archiveContainer = document.getElementById('archive-container');

  // Renders a single archived note into the modal
  const renderArchivedNote = (note) => {
    const noteDiv = document.createElement('div');
    noteDiv.classList.add('archived-note');

    const content = document.createElement('p');
    content.textContent = note.content;

    const buttonContainer = document.createElement('div');
    buttonContainer.classList.add('archived-buttons');
    ...
    restoreBtn.addEventListener('click', async () => {
      ...
      noteDiv.remove(); // Remove from archive view
      loadPostIts(); // Reload the main post-it list
    });
  ...
  // Fetches archived notes and displays them in the modal
  const loadArchivedNotes = async () => {
    archiveContainer.innerHTML = 'Loading...';
    const { data: notes, error } = await supabaseClient
      .from('post_its')
      .select('*')
      .eq('is_archived', true)
    ...
    if (error) {
      archiveContainer.innerHTML = 'Error loading archive.';
      return;
    }
    ...
    if (notes.length === 0) {
      ...
      notes.forEach(note => renderArchivedNote(note));
    }
  };
